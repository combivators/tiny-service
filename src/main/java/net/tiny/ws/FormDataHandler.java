...
public abstract class FormDataHandler extends BaseWebService {
    ...
    protected static final Logger LOGGER = Logger.getLogger(FormDataHandler.class.getName());
    ...
    protected void execute(HTTP_METHOD method, HttpExchange httpExchange) throws IOException {
        ...
        if (request.hasMultipart()) {
            ...
            byte[] payload = readContents(httpExchange.getRequestBody());
     ...
     */
    public List<Integer> searchBytes(byte[] source, byte[] target, int start, int end) {
     ...
     * @return
     */
    public int indexOf(byte[] source, byte[] target, int start, int end) {
